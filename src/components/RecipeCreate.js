...
class RecipeCreate extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.user) {
      this.setState({ author: nextProps.user.displayName, authorId: nextProps.userId})
    }
  }
  ...
  saveNewRecipe(e) {
    ...
    var postData = {
      authorId, author, title, ingredients, instructions, timeCreated: Date.now()
    };
    ...
    return firebase.database().ref().update(updates).then((err) => {
      if (err) {
        console.log('nuts')
      } else {
  ...
  render() {
    ...
    return (
      <div>
        <form onSubmit={this.saveNewRecipe.bind(this)}>
